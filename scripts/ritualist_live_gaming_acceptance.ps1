...
public static class RitualistLiveWindows {
...
"@ -ErrorAction SilentlyContinue | Out-Null
    ...
    $foreground = [RitualistLiveWindows]::GetForegroundWindow()
    $callback = [RitualistLiveWindows+EnumWindowsProc]{
        param([IntPtr]$hWnd, [IntPtr]$lParam)
        if ([RitualistLiveWindows]::IsWindowVisible($hWnd)) {
            $titleBuilder = New-Object System.Text.StringBuilder 512
            $classBuilder = New-Object System.Text.StringBuilder 256
            [void][RitualistLiveWindows]::GetWindowText($hWnd, $titleBuilder, $titleBuilder.Capacity)
            [void][RitualistLiveWindows]::GetClassName($hWnd, $classBuilder, $classBuilder.Capacity)
            $pid = [uint32]0
            [void][RitualistLiveWindows]::GetWindowThreadProcessId($hWnd, [ref]$pid)
    ...
    [void][RitualistLiveWindows]::EnumWindows($callback, [IntPtr]::Zero)
...
function Invoke-RitualistCommand {
    param(
        [string]$Id,
        [string[]]$Arguments
    )
...
function New-CaseRecord {
    ...
    $message = "Live evidence captured for human review. Compare the desktop state and Ritualist logs to the case pass_when rule."
    ...
    elseif ($Definition.id -eq "diablo_already_running" -and $diabloPresent) {
        $message = "Diablo process/window was observed; human review must confirm Ritualist completed without Play approval."
    }
...
$runsEvidence = Invoke-RitualistCommand "runs-no-repair" @("-m", "ritualist", "runs", "--limit", "10", "--no-repair")
...
$summary = [ordered]@{
    schema = "ritualist.live_gaming_acceptance_summary.v1"
...
$lines = @(
    "# Ritualist Live Gaming Acceptance",
    "",
    "- Release: v0.2.0-alpha.1",
