...
public static class SetpieceAcceptanceWin32 {
...
$FakeWallpaperTitle = "Setpiece Wallpaper Fixture"
...
function Capture-ScreenFrames {
    ...
    Write-JsonFile $manifest ([ordered]@{
        schema = "setpiece.acceptance.frame_recording.v1"
        name = $Name
        frames = $frames
        encoded_video = $null
...
function Get-TopLevelWindows {
    ...
    $callback = [SetpieceAcceptanceWin32+EnumWindowsProc]{
        param([System.IntPtr]$hWnd, [System.IntPtr]$lParam)
        if ([SetpieceAcceptanceWin32]::IsWindowVisible($hWnd)) {
            $handles.Add($hWnd)
        }
        return $true
    }
    [SetpieceAcceptanceWin32]::EnumWindows($callback, [System.IntPtr]::Zero) | Out-Null
    $foreground = [SetpieceAcceptanceWin32]::GetForegroundWindow()
    $rows = @()
    $index = 0
    foreach ($handle in $handles) {
        $length = [SetpieceAcceptanceWin32]::GetWindowTextLength($handle)
        $builder = New-Object System.Text.StringBuilder ([Math]::Max(1, $length + 1))
        [SetpieceAcceptanceWin32]::GetWindowText($handle, $builder, $builder.Capacity) | Out-Null
        $title = $builder.ToString()
        if ([string]::IsNullOrWhiteSpace($title)) {
            continue
        }
        [uint32]$processId = 0
        [SetpieceAcceptanceWin32]::GetWindowThreadProcessId($handle, [ref]$processId) | Out-Null
        $rect = New-Object SetpieceAcceptanceWin32+RECT
        $bounds = $null
        if ([SetpieceAcceptanceWin32]::GetWindowRect($handle, [ref]$rect)) {
            $bounds = [ordered]@{
                x = [int]$rect.Left
                y = [int]$rect.Top
...
function Set-ForegroundProcessWindow {
    ...
    for ($attempt = 0; $attempt -lt 10; $attempt += 1) {
        ...
        if ($handle -ne [System.IntPtr]::Zero) {
            [void][SetpieceAcceptanceWin32]::ShowWindow($handle, 9)
            [void][SetpieceAcceptanceWin32]::SetForegroundWindow($handle)
...
function Start-AcceptanceProcess {
    ...
    $startInfo.EnvironmentVariables["SETPIECE_E2E_ARTIFACT_DIR"] = $E2ERoot
    $startInfo.EnvironmentVariables["SETPIECE_E2E_APP_DATA_DIR"] = $script:FixtureAppData
...
function Invoke-PositiveFakeTargetApprovalEvidence {
    ...
    try {
        ...
        $target = if ($null -ne $window) { Find-Button $window "Play" } else { $null }
        $targetEvidence = Get-ElementEvidence $target
        $simulatedApproval = $null -ne $target -and [bool]$target.Current.IsEnabled
        $invoked = $false
        ...
        $tree = if ($null -ne $window) { Save-WindowTree "positive-fake-target-approval" $window } else { $null }
...
function New-ProjectRoomShortcutFixture {
    ...
    $source = Join-Path $RepoRoot "setpiece\sample_canvases\project_room.yaml"
...
function Invoke-RoomPickerEvidence {
    $process = Start-AcceptanceProcess $script:SetpieceExe @()
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Home" 10
        ...
        $homeRow = $zRows | Where-Object { [int64]$_.process_id -eq [int64]$process.Id -and $_.title -eq "Setpiece Home" } | Select-Object -First 1
        ...
        $roomList = Invoke-CapturedCommand "room-list" "python" @("-m", "setpiece", "room", "list", "--json")
...
from setpiece.canvas import CanvasRuntimeContext, build_canvas_runtime_model, load_bundled_canvas
...
payload = {
    "schema": "setpiece.acceptance.state_ui_fixture.v1",
    "states": states,
    "fixtures": fixtures,
    "forbidden_capture": {
        "screenshots": False,
...
function Invoke-HeroRoomEvidence {
    ...
    $projectProcess = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", $script:ProjectRoomShortcutFixtureCanvas)
    $projectEvidence = @{}
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Canvas" 10
        ...
        $roomShow = Invoke-CapturedCommand "project-room-show" "python" @("-m", "setpiece", "room", "show", "project", "--json")
        $doctor = Invoke-CapturedCommand "coding-mode-doctor" "python" @("-m", "setpiece", "doctor", "setpiece\sample_recipes\coding_mode.yaml", "--json", "--no-strict")
        $dryRun = Invoke-CapturedCommand "coding-mode-dry-run" "python" @("-m", "setpiece", "dry-run", "setpiece\sample_recipes\coding_mode.yaml")
        $beforeRuns = Get-RunDirectoryNames
        $folderDryRun = Invoke-CapturedCommand "project-folder-shortcut-dry-run" "python" @("-m", "setpiece", "canvas", "action", $script:ProjectRoomShortcutFixtureCanvas, "project_folder", "open", "--dry-run", "--json")
    ...
    $supportProcess = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", "helpdesk_desktop")
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Canvas" 10
        ...
        $roomShow = Invoke-CapturedCommand "support-desk-show" "python" @("-m", "setpiece", "room", "show", "support_desk", "--json")
        $diagnosticsDryRun = Invoke-CapturedCommand "support-diagnostics-dry-run" "python" @("-m", "setpiece", "dry-run", "setpiece\sample_recipes\collect_basic_diagnostics.yaml")
        $audioDryRun = Invoke-CapturedCommand "support-audio-dry-run" "python" @("-m", "setpiece", "dry-run", "setpiece\sample_recipes\meeting_audio_troubleshooting.yaml")
        $roomPayload = Convert-CommandJson $roomShow
        $runbookCards = @()
        if ($roomPayload) {
            $runbookCards = @(
        ...
        $vpnText = Get-Content -Path (Join-Path $RepoRoot "setpiece\sample_recipes\vpn_repair_placeholder.yaml") -Raw
        $newHireText = Get-Content -Path (Join-Path $RepoRoot "setpiece\sample_recipes\new_hire_setup_draft.yaml") -Raw
...
function Set-GamingRoomAggregateEvidence {
    $roomShow = Invoke-CapturedCommand "gaming-room-show" "python" @("-m", "setpiece", "room", "show", "gaming", "--json")
    $required = @(
...
function New-Fixtures {
    ...
    $script:FixtureAppData = Join-Path $script:FixtureLocalAppData "Setpiece\Setpiece"
...
$form = New-Object System.Windows.Forms.Form
$form.Text = "Setpiece Wallpaper Fixture"
...
function Resolve-SetpieceExe {
    if ($ExecutablePath) {
        return (Resolve-Path $ExecutablePath).Path
    }
    $candidate = Join-Path $RepoRoot "dist\Setpiece\Setpiece.exe"
...
function Assert-LaunchWindow {
    ...
    $process = Start-AcceptanceProcess -FilePath $script:SetpieceExe -Arguments $LaunchArguments
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        ...
        if ($null -ne $window -and -not $process.HasExited -and (Test-ScreenshotNonBlank $screenshot)) {
...
function Capture-CanvasVisualArtifact {
    ...
    $process = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", $CanvasId)
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Canvas" 10
...
function Capture-DesktopWorkAreaCanvasArtifact {
    ...
    try {
        ...
        $process = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", "minimal_desktop", "--host", "desktop-work-area")
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Canvas" 10
        $fakeWallpaperWindow = Get-WindowByName $FakeWallpaperTitle 2
        $focusedCanvas = Set-ForegroundProcessWindow $process "Setpiece Canvas"
        ...
        $screen = Get-PrimaryScreenGeometry
        $windowRow = @($zOrder | Where-Object { [int64]$_.process_id -eq [int64]$process.Id -and $_.title -eq "Setpiece Canvas" } | Select-Object -First 1)
        ...
        $exitInvoked = if ($null -ne $window) { Invoke-NamedButton $window "Exit Desktop Canvas" 5 } else { $false }
        Start-Sleep -Seconds 2
...
function Capture-DesktopWorkAreaWindowedFallbackArtifact {
    $process = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", "minimal_desktop", "--host", "desktop-work-area") @{
        SETPIECE_CANVAS_FORCE_WINDOWED = "1"
    }
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Canvas" 10
        ...
        Add-VisualArtifact -Id "desktop-work-area-windowed-fallback" -CanvasId "minimal_desktop" -State "forced_windowed" -NonBlank (Test-ScreenshotNonBlank $screenshot) -Evidence @{
            ...
            window_tree = $windowTree
            z_order = $zOrderPath
            force_windowed_env = "SETPIECE_CANVAS_FORCE_WINDOWED"
            host_ready = if ($hostReady.Count -gt 0) { $hostReady[-1] } else { $null }
...
function Capture-HeroDesktopWorkAreaArtifacts {
    ...
    try {
        ...
        foreach ($hero in $heroes) {
            $process = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", $hero.canvas_id, "--host", "desktop-work-area")
            try {
                Start-Sleep -Seconds $ScenarioDwellSeconds
                $window = Get-WindowByName "Setpiece Canvas" 10
                $fakeWallpaperWindow = Get-WindowByName $FakeWallpaperTitle 2
...
function Capture-CanvasEditModeVisualArtifact {
    $process = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", "gaming_desktop")
    try {
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $window = Get-WindowByName "Setpiece Canvas" 10
        $editInvoked = if ($null -ne $window) { Invoke-NamedButton $window "Edit Room" 10 } else { $false }
        Start-Sleep -Seconds $ScenarioDwellSeconds
        $editWindow = Get-WindowByName "Setpiece Canvas" 5
...
function Invoke-CanvasStaticActions {
    ...
    try {
        ...
        $window = Get-WindowByName "Setpiece Canvas" 15
        ...
        $buttons = @($requiredButtons | Where-Object {
            $null -ne $window -and (Find-AnyButton $window $_.names)
        } | ForEach-Object { $_.id })
        $rendered = $null -ne $window -and (Test-ScreenshotNonBlank $initial) -and $buttons.Count -ge 4
        ...
        $previewCommand = Invoke-CapturedCommand "target-preview" "python" @(
            "-m", "setpiece", "canvas", "action", "gaming_desktop", "diablo_target", "preview_plan", "--json"
        )
        ...
        $rootHelp = Invoke-CapturedCommand "cli-help-no-recording-surface" "python" @("-m", "setpiece", "--help")
        $canvasHelp = Invoke-CapturedCommand "canvas-help-no-recording-surface" "python" @("-m", "setpiece", "canvas", "--help")
...
function Invoke-CanvasRunDecline {
    ...
    try {
        ...
        $window = Get-WindowByName "Setpiece Canvas" 15
        [void](Invoke-AnyNamedButton $window @("run", "Run") 10)
        $confirmation = Get-WindowByName "Setpiece Confirmation Required" 60
        ...
        $cancel = if ($null -ne $confirmation) { Invoke-AnyNamedButton $confirmation @("Not now", "Cancel", "Cancel Ritual") 5 } else { $false }
        ...
        $runEvidence = if ($run) { Copy-RunLogEvidence $run.Name } else { $null }
        $show = if ($run) {
            Invoke-CapturedCommand "show-run-declined" "python" @("-m", "setpiece", "show-run", $run.Name, "--no-repair")
        }
        else {
            $null
        }
        $zRows = Get-Content -Path $zOrder -Raw | ConvertFrom-Json
        $confirmationIndex = ($zRows | Where-Object { $_.title -eq "Setpiece Confirmation Required" } | Select-Object -First 1).z_index
        ...
        if ($null -ne $confirmation -and $confirmationAbove) {
        ...
        $runsCommand = if ($run) {
            Invoke-CapturedCommand "runs-after-decline" "python" @("-m", "setpiece", "runs", "--limit", "5", "--no-repair")
        }
        else {
            $null
        }
...
function Invoke-HardKillRecovery {
    ...
    try {
        ...
        $window = Get-WindowByName "Setpiece Canvas" 15
        [void](Invoke-AnyNamedButton $window @("run", "Run") 10)
        $confirmation = Get-WindowByName "Setpiece Confirmation Required" 35
        Start-Sleep -Seconds 1
        ...
        $process = $null
        Start-Sleep -Seconds 2
        $homeProcess = Start-AcceptanceProcess $script:SetpieceExe @()
        Start-Sleep -Seconds 5
        ...
        $show = if ($runId) {
            Invoke-CapturedCommand "show-run-interrupted" "python" @("-m", "setpiece", "show-run", $runId, "--no-repair")
        }
        else {
            $null
        }
...
payload = {
    "schema": "setpiece.acceptance.north_star_journal_events.v1",
...
from setpiece.activity_collectors import FakeActivityCollector
from setpiece.activity_signals import journal_event_signal, recent_reference_signal
...
from setpiece.paths import recipes_path
...
from setpiece.suggestions.review import approve_suggestion
...
payload = {
    "schema": "setpiece.acceptance.north_star_suggestion_drafts.v1",
...
'@ | Set-Content -Path $draftProbe -Encoding UTF8
    ...
    $enable = Invoke-CapturedCommand "north-star-learning-enable" "python" @("-m", "setpiece", "learning", "enable", "--source", "setpiece_journal", "--source", "recent_items", "--json")
    $journalWrite = Invoke-CapturedCommand "north-star-journal-events" "python" @($journalProbe, $journalOutput)
    $journal = Invoke-CapturedCommand "north-star-learning-journal" "python" @("-m", "setpiece", "learning", "journal", "--json")
    $learningScan = Invoke-CapturedCommand "north-star-learning-scan" "python" @("-m", "setpiece", "learning", "scan", "--json")
    $suggestionScan = Invoke-CapturedCommand "north-star-suggestions-scan" "python" @($suggestionProbe, $suggestionOutput)
    $suggestionList = Invoke-CapturedCommand "north-star-suggestions-list" "python" @("-m", "setpiece", "suggestions", "list", "--json")
    $drafts = Invoke-CapturedCommand "north-star-suggestion-drafts" "python" @($draftProbe, $draftOutput, $reviewedFolder)
    $delete = Invoke-CapturedCommand "north-star-learning-delete-data" "python" @("-m", "setpiece", "learning", "delete-data", "--yes", "--json")
    ...
    $shortcutDraftOk = (
        $draftPayload -and
        $draftPayload.shortcut_draft.schema_version -eq "setpiece.shortcut_draft.v1" -and
    ...
    $recipeDraftOk = (
        $draftPayload -and
        $draftPayload.recipe_draft.schema_version -eq "setpiece.suggestion.recipe_draft.v1" -and
...
function Invoke-PackSafetyChecks {
    $canvasOut = Join-Path $CommandRoot "visual_acceptance.setpiececanvas"
    $themeOut = Join-Path $CommandRoot "minimal_theme.setpiecetheme"
    ...
    $canvasExport = Invoke-CapturedCommand "canvas-pack-export" "python" @("-m", "setpiece", "canvas", "pack", "export", $canvasPath, "--out", $canvasOut, "--json")
    $canvasImport = Invoke-CapturedCommand "canvas-pack-import" "python" @("-m", "setpiece", "canvas", "pack", "import", $canvasOut, "--json")
    $themeExport = Invoke-CapturedCommand "theme-pack-export" "python" @("-m", "setpiece", "canvas", "theme", "export", $themePath, "--out", $themeOut, "--json")
    $themeImport = Invoke-CapturedCommand "theme-pack-import" "python" @("-m", "setpiece", "canvas", "theme", "import", $themeOut, "--json")
...
"@ | Set-Content -Path $badCanvas -Encoding UTF8
    $badOut = Join-Path $CommandRoot "bad_component.setpiececanvas"
    $badExport = Invoke-CapturedCommand "bad-component-export" "python" @("-m", "setpiece", "canvas", "pack", "export", $badCanvas, "--out", $badOut, "--json")
...
function Invoke-SuitePackQuarantineEvidence {
    $suiteCanvasOut = Join-Path $CommandRoot "north_star_suite_canvas.setpiececanvas"
    $suiteThemeOut = Join-Path $CommandRoot "north_star_suite_theme.setpiecetheme"
    $suiteRitualRecipe = Join-Path $CommandRoot "north_star_suite_wait.yaml"
    $suiteRitualOut = Join-Path $CommandRoot "north_star_suite_wait.setpiecepack"
    $suiteOut = Join-Path $CommandRoot "north_star_suite.setpiecesuite"
...
"@ | Set-Content -Path $suiteRitualRecipe -Encoding UTF8
    ...
    $runsBefore = Get-RunDirectoryNames
    $canvasExport = Invoke-CapturedCommand "north-star-suite-canvas-export" "python" @("-m", "setpiece", "canvas", "pack", "export", (Join-Path $script:FixtureAppData "canvases\visual_acceptance.yaml"), "--out", $suiteCanvasOut, "--json")
    $themeExport = Invoke-CapturedCommand "north-star-suite-theme-export" "python" @("-m", "setpiece", "canvas", "theme", "export", (Join-Path $script:FixtureAppData "themes\minimal_theme.yaml"), "--out", $suiteThemeOut, "--json")
    $ritualExport = Invoke-CapturedCommand "north-star-suite-ritual-export" "python" @("-m", "setpiece", "pack", "export", $suiteRitualRecipe, "--out", $suiteRitualOut)
    $suiteExport = Invoke-CapturedCommand "north-star-suite-export" "python" @("-m", "setpiece", "suite", "export", "--canvas-pack", $suiteCanvasOut, "--theme-pack", $suiteThemeOut, "--ritual-pack", $suiteRitualOut, "--out", $suiteOut, "--id", "north_star_suite", "--name", "North Star Suite", "--readme", $readme, "--json")
    $suiteValidate = Invoke-CapturedCommand "north-star-suite-validate" "python" @("-m", "setpiece", "suite", "validate", $suiteOut, "--json")
    $suiteImport = Invoke-CapturedCommand "north-star-suite-import" "python" @("-m", "setpiece", "suite", "import", $suiteOut, "--json")
    $suiteList = Invoke-CapturedCommand "north-star-suite-list-imports" "python" @("-m", "setpiece", "suite", "list-imports", "--json")
    $runsAfter = Get-RunDirectoryNames
...
function Invoke-PerformanceChecks {
    $perf100 = Invoke-CapturedCommand "perf-100" "python" @("-m", "setpiece", "perf", "canvas-use", "--mock-components", "100", "--json")
    $perf300 = Invoke-CapturedCommand "perf-300" "python" @("-m", "setpiece", "perf", "canvas-use", "--mock-components", "300", "--json")
    $heartbeatProcess = Start-AcceptanceProcess $script:SetpieceExe @("--canvas", "gaming_desktop")
    ...
    try {
        Start-Sleep -Seconds 5
        $heartbeatWindow = Get-WindowByName "Setpiece Canvas" 15
...
function Set-RemainingReviewChecks {
    if (-not $Results.Contains("recent_activity_updates")) {
        $runsCommand = Invoke-CapturedCommand "runs-after-acceptance" "python" @("-m", "setpiece", "runs", "--limit", "5", "--no-repair")
...
function Write-Summaries {
    ...
    $summary = [ordered]@{
        schema = "setpiece.release_acceptance_summary.v1"
        release = "v0.2.0-alpha.1"
        acceptance_spec = $AcceptanceSpec
        generated_at = (Get-Date).ToUniversalTime().ToString("o")
        repo_root = $RepoRoot
        packaged = [bool]$Packaged
        executable = $script:SetpieceExe
        command_scope = "Packaged executable is used for Home, Canvas Use Mode, classic GUI, and runtime scenarios. Source-tree python -m setpiece is used for supplemental CLI-only safety, perf, run-log, and visual-pack command evidence because the Windows app bundle is a GUI entry point."
    ...
    $lines = @()
    $lines += "# Setpiece v0.2.0-alpha.1 Acceptance Summary"
    $lines += ""
...
try {
    New-Fixtures
    $script:SetpieceExe = Resolve-SetpieceExe
    Assert-LaunchWindow -Id "packaged_home_visible" -Title "Setpiece Home" -LaunchArguments @() -ScreenshotName "packaged-home"
    Invoke-RoomPickerEvidence
    Assert-LaunchWindow -Id "packaged_canvas_visible" -Title "Setpiece Canvas" -LaunchArguments @("--canvas", "gaming_desktop") -ScreenshotName "packaged-canvas"
    Assert-LaunchWindow -Id "packaged_classic_gui_visible" -Title "Setpiece" -LaunchArguments @("--classic-gui") -ScreenshotName "packaged-classic-gui"
