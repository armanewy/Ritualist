...
$SummarySchema = "ritualist.ui_migration_baseline_summary.v1"
...
public static class RitualistUiBaselineWin32 {
...
function Get-TopLevelWindows {
    ...
    $callback = [RitualistUiBaselineWin32+EnumWindowsProc]{
        param([IntPtr]$hWnd, [IntPtr]$lParam)
        if (-not [RitualistUiBaselineWin32]::IsWindowVisible($hWnd)) {
            return $true
        }

        $titleBuilder = New-Object System.Text.StringBuilder 512
        $classBuilder = New-Object System.Text.StringBuilder 256
        [void][RitualistUiBaselineWin32]::GetWindowText($hWnd, $titleBuilder, $titleBuilder.Capacity)
        [void][RitualistUiBaselineWin32]::GetClassName($hWnd, $classBuilder, $classBuilder.Capacity)
        ...
        [uint32]$processId = 0
        [void][RitualistUiBaselineWin32]::GetWindowThreadProcessId($hWnd, [ref]$processId)
        $rect = New-Object RitualistUiBaselineWin32+RECT
        $bounds = $null
        if ([RitualistUiBaselineWin32]::GetWindowRect($hWnd, [ref]$rect)) {
            $bounds = [ordered]@{
                x = [int]$rect.Left
                y = [int]$rect.Top
        ...
        $exStyle = [RitualistUiBaselineWin32]::GetWindowLongPtr($hWnd, -20).ToInt64()
    ...
    [void][RitualistUiBaselineWin32]::EnumWindows($callback, [IntPtr]::Zero)
...
function Start-BaselineProcess {
    ...
    $startInfo.EnvironmentVariables["RITUALIST_E2E_ARTIFACT_DIR"] = $script:E2ERoot
    $startInfo.EnvironmentVariables["RITUALIST_E2E_APP_DATA_DIR"] = $script:AppDataRoot
...
function Get-E2EEvents {
    ...
    $events = @()
    Get-ChildItem -LiteralPath $script:E2ERoot -Filter "ritualist-e2e-*.jsonl" -ErrorAction SilentlyContinue |
        ForEach-Object {
...
function Write-SummaryMarkdown {
    ...
    $lines = @()
    $lines += "# Ritualist UI migration baseline"
    $lines += ""
    ...
    $lines += ""
    $lines += "## Observations"
    $lines += "- Visible Ritualist window count: ``$($Summary.observations.visible_window_count)``"
...
$resolvedExecutable = if ([string]::IsNullOrWhiteSpace($ExecutablePath)) {
    Join-Path $RepoRoot "dist\Ritualist\Ritualist.exe"
}
...
try {
    ...
    elseif (-not (Test-IsWindowsHost)) {
        ...
        $homeVisible = [bool]($homeTopWindows | Where-Object { $_.title -like "*Ritualist*" } | Select-Object -First 1)
        ...
        $ritualistProcessIds = @(Get-DescendantProcessIds $rootProcessIds)
        $processRows = @(Get-ProcessTreeRows $rootProcessIds)
        $topLevelWindows = @(Get-TopLevelWindows | Where-Object { $ritualistProcessIds -contains [int]$_.process_id })
        $windowTrees = @(Get-UiaWindowTrees $ritualistProcessIds)
        ...
        $homeStillVisible = [bool]($topLevelWindows | Where-Object { $_.process_id -eq [int]$homeProcess.Id -and $_.title -like "*Ritualist*" } | Select-Object -First 1)
