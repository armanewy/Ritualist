...
Setpiece turns Windows into activity-specific Rooms powered by safe local rituals that can be previewed, checked, run, paused, logged, and recovered. A ritual is a readable YAML recipe with explicit steps, validation, dry-run support, logs, recovery, and confirmation gates for risky actions.

Setpiece is a local ritual/runbook engine with a desktop-native body. Recipes and rituals remain the center of gravity; Rooms, Canvas, shortcuts, packs, and future Suggestions exist to make rituals more visible, useful, and trustworthy. Setpiece is not a shell replacement, file manager, wallpaper engine, generic widget platform, general RPA suite, macro recorder, Recall clone, marketplace, cloud automation service, remote execution tool, password automation tool, or gameplay automation tool.

This alpha implementation includes:

- CLI runner: `setpiece run recipe.yaml`
- Recipe initialization and discovery: `setpiece init`, `setpiece list`, `setpiece paths`
- Portable recipe packs: `setpiece pack export`, `setpiece pack import`, `setpiece pack list-imports`, `setpiece pack enable`
- Desktop diagnostics: `setpiece doctor`, `setpiece inspect-window`
- GUI launcher: `setpiece gui`
- Canvas schema and component kernel: `setpiece canvas list`, `setpiece canvas validate`
...
setpiece init
setpiece list
setpiece validate gaming_mode
setpiece doctor gaming_mode
setpiece doctor gaming_mode --json
setpiece dry-run gaming_mode
setpiece run gaming_mode --var youtube_url=https://www.youtube.com/watch?v=...
```
...
```powershell
setpiece pack export gaming_mode --out gaming_mode.setpiecepack
setpiece pack import .\gaming_mode.setpiecepack
setpiece pack list-imports
setpiece doctor <quarantined-recipe-path>
setpiece dry-run <quarantined-recipe-path>
```

Pack export writes a `.setpiecepack` zip containing `manifest.yaml`, `recipe.yaml`, and
...
Canvas documents are local YAML layouts made of native Setpiece components bound to recipes, intents, targets, runtime state, and static display data. Canvas validation is side-effect free; it does not run recipes, launch apps, click, type, open browsers, call UI Automation, or execute bindings.

```powershell
python -m setpiece canvas init
python -m setpiece canvas list
python -m setpiece canvas validate gaming_desktop
python -m setpiece canvas show gaming_desktop --json
...
python -m setpiece room show gaming --json
```
...
- Launch `dist\Setpiece\Setpiece.exe`.
- Open Home.
...
```powershell
setpiece inspect-window "Battle.net" --control-type Button --limit 100
```
...
python -m setpiece doctor gaming_mode
python -m setpiece dry-run gaming_mode
python -m setpiece inspect-window "Battle.net"
python -m setpiece run gaming_mode
```
...
The sample recipe sets `keep_open: true` on `browser.open`, so after the workflow reaches the browser step, the Setpiece CLI stays alive even if a later desktop step fails or the final Play confirmation is cancelled. Press `Ctrl+C` to exit the Setpiece CLI and let Playwright close its browser process.

## Starter Workspace Templates

Bundled starter templates live under `setpiece\sample_recipes` for local editing and review:
...
```powershell
python -m setpiece validate .\setpiece\sample_recipes\coding_mode.yaml
python -m setpiece dry-run .\setpiece\sample_recipes\coding_mode.yaml
```
...
```powershell
$env:SETPIECE_RUNTIME_SMOKE = "1"
python -m pytest tests\test_runtime_smoke.py
Remove-Item Env:\SETPIECE_RUNTIME_SMOKE
```

## Performance

See [PERFORMANCE.md](PERFORMANCE.md) for Setpiece's UI responsiveness contract, runtime event rules, and performance budgets. See [RUNTIME.md](RUNTIME.md) for Runtime v2 run states, step states, events, controls, waits, and GUI/Home integration rules.
...
The GUI shows a best-effort transparent overlay before window focus/minimize/maximize and desktop click actions. For `desktop.click_text`, Setpiece previews the UI Automation element bounds when Windows exposes them, then keeps the existing confirmation gate for risky actions such as `Play`. Long `window.wait` steps show a small waiting HUD with elapsed seconds.
...
Use `setpiece paths` to inspect local directories. Setpiece creates:
...
Per-run logs are written to `runs/<timestamp>_<recipe_id>/run.json` and `steps.jsonl`. Add user-entered operator notes during or after a run with `setpiece note-run <run-id-or-path> "note text"`; notes are stored in the run folder as `operator_notes.jsonl` and are shown by `setpiece show-run`. Browser URLs are redacted in run step messages; Setpiece does not log cookies, screenshots, page contents, passwords, or secrets.
...
Run history uses `success` for completed runs, `stopped` for cleanly cancelled or failed workflows, and `interrupted` when a previous Setpiece process exited before finalizing `run.json`. `setpiece runs` repairs stale `running` records automatically; use `setpiece runs --no-repair` to inspect raw statuses without reconciliation.

## Browser Lifecycle

Playwright owns the browser process it launches. When a CLI run exits, the browser it opened may close with the Playwright driver. For media workflows, set `keep_open: true` on `browser.open` or pass `setpiece run <recipe> --keep-alive`; Setpiece will keep the CLI process alive after execution until you press `Ctrl+C`. Recipe-level `keep_open: true` activates only after that browser step succeeds. The `--keep-alive` option keeps the CLI alive after execution regardless of workflow success, unless the run is a dry-run.
...
Setpiece-managed profile folders under the local `browser-profiles` directory.
...
release treats it as a safe no-op unless Setpiece has a browser UI-scoped
mechanism available. Setpiece does not use webpage text or buttons to dismiss
Chrome restore prompts, because page content can imitate browser prompts.
`use_dedicated_profile: false` is rejected so Setpiece cannot silently operate
...
create a Setpiece browser profile, and does not claim DOM or media control.
It is still logged as a ritual step and is supported by dry-run and Doctor.

Setpiece's managed Playwright launch options are audited by tests to ensure
...
Setpiece supports a narrow browser runbook surface for pages it opened through
`browser.open`. These actions operate on the current Setpiece-managed page:
...
role plus `accessible_name`, or test id. Setpiece does not support arbitrary
...
steps:
  ...
  - action: human.prompt
    prompt: Sign in manually if the page asks, then return to Setpiece.
...
This pattern keeps credentials with the user and the browser profile. Setpiece
only waits for structured page state or clicks explicit, reviewable controls.

## Building A Local Windows App

Use a PyInstaller one-folder build for alpha packaging. One-folder mode leaves the executable and its support files visible under `dist\Setpiece`, which makes missing data files and DLL issues easier to diagnose than a one-file bundle.
...
If `dist\Setpiece\Setpiece.exe` opens but a workflow fails, launch `dist\Setpiece\Setpiece.exe --classic-gui` and use **About / Diagnostics** first. It shows whether the app is running from a PyInstaller bundle, where user data/logs/runs/browser profiles live, and whether PySide6, Playwright, and Windows UI Automation dependencies are importable. Use **Copy Diagnostics** when filing a bug diary entry.
...
If Battle.net cannot launch, run **Doctor** for the selected recipe or `python -m setpiece doctor gaming_mode`. Missing-path errors mean the installed recipe variable/config points at a local file that does not exist on this machine.

If UI Automation labels are not found, use the development CLI to inspect the live window:

```powershell
python -m setpiece inspect-window "Battle.net" --control-type Button --limit 100
```

Then update `desktop.click_text` labels in the installed recipe. Setpiece still does not use coordinate clicks or gameplay automation.

Packaged startup failures are written to `startup-error.log` under the logs directory when possible. Normal app logs are under `logs`, and run details are under `runs`; the exact locations are shown in **About / Diagnostics** and by `python -m setpiece paths`.

## Diagnostics

`setpiece doctor <recipe-id-or-path>` validates a recipe without opening browsers, launching apps, clicking, typing, or running workflow actions. It checks optional dependency availability, OS support, browser profile creation, local app paths, `environment` contracts, and the window/text targets for `desktop.click_text`. Environment expected-window and expected-label checks use read-only inspection only; they are portability hints, not automation steps.
...
`setpiece inspect-window <title-contains>` is Windows-only and prints matching window titles plus visible descendant labels. Use it to discover the exact labels exposed to UI Automation before editing `desktop.click_text` steps.
