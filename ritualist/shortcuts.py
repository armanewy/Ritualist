...
class ShortcutService:
    ...
    def open(self, request: ShortcutRequest) -> ShortcutResult:
        ...
        if request.kind is ShortcutKind.FOLDER:
            ...
            raise RitualistError(f"unsupported shortcut kind: {request.kind}")
        record_event(
            "shortcut.opened",
...
def shortcut_request_from_component(component: Any) -> ShortcutRequest:
    ...
    if kind is None:
        raise RitualistError(f"{component.id}: component is not a shortcut")
