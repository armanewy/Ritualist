...
from ritualist.runtime_models import RunState, StepState
...
def _tray_tooltip(state: AgentState) -> str:
    ...
    if state.state == AgentRunState.IDLE:
        return "Ritualist - Ready"
    if label:
        return f"Ritualist - {_state_label(state.state)}: {label}"
    return f"Ritualist - {_state_label(state.state)}"
