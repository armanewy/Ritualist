...
from setpiece.models import SAFE_ID_PATTERN
from setpiece.paths import browser_profiles_dir
...
class PlaywrightBrowserAdapter:
    ...
    def configure_media(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser.media requires a prior browser.open step")
    ...
    def media_playing(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser.wait_media_playing requires a prior browser.open step")
        try:
            from playwright.sync_api import TimeoutError as PlaywrightTimeoutError
        except ImportError as exc:
            raise DependencyMissingError(
                "browser media checks require Playwright; install setpiece[browser] and run "
    ...
    def text_visible(self, *, text: str, exact: bool, timeout_seconds: float) -> bool:
        if self._page is None:
            raise SetpieceError("assert.browser_text_visible requires a prior browser.open step")
        try:
            from playwright.sync_api import TimeoutError as PlaywrightTimeoutError
        except ImportError as exc:
            raise DependencyMissingError(
                "browser assertions require Playwright; install setpiece[browser] and run "
                "'python -m playwright install chromium'"
            ) from exc
    ...
    def title_matches(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser.wait_title requires a prior browser.open step")
    ...
    def url_matches(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser.wait_url requires a prior browser.open step")
    ...
    def element_visible(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser.element_visible requires a prior browser.open step")
    ...
    def click_text(self, *, text: str, exact: bool, timeout_seconds: float) -> None:
        if self._page is None:
            raise SetpieceError("browser.click_text requires a prior browser.open step")
        locator = self._page.get_by_text(text, exact=exact).first
        locator.wait_for(state="visible", timeout=timeout_seconds * 1000)
    ...
    def click_role(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser.click_role requires a prior browser.open step")
    ...
    def click_test_id(self, *, test_id: str, timeout_seconds: float) -> None:
        if self._page is None:
            raise SetpieceError("browser.click_test_id requires a prior browser.open step")
        ...
        *,
        ...
        if not use_dedicated_profile:
            raise SetpieceError(
                ...
                "Setpiece only opens managed browser profiles"
            )
        ...
        try:
            ...
            raise DependencyMissingError(
                "browser actions require Playwright; install setpiece[browser] and run "
                "'python -m playwright install chromium'"
            ) from exc
        ...
        if self._context is None:
            ...
            if browser == "chrome":
                ...
                raise SetpieceError(f"unsupported browser '{browser}'")
    ...
    def _structured_locator(
        ...
        *,
        ...
        if self._page is None:
            raise SetpieceError("browser action requires a prior browser.open step")
        if text is not None:
        ...
        raise SetpieceError("browser action requires a structured target")
