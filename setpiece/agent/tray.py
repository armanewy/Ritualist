...
from setpiece.brand_assets import qt_icon_from_brand_asset, select_tray_variant, tray_icon_asset
...
class AgentTray:
    ...
    current_context: TrayContext = field(default_factory=lambda: TrayContext(TrayState.READY))
    current_variant: str = "light"
    ...
    def set_ready(self) -> None:
        self.set_state(TrayContext(TrayState.READY))

    def set_state(self, context: TrayContext, *, variant: str | None = None) -> None:
        self.current_context = context
        self.current_variant = variant or select_tray_variant(self.app)
        model = build_tray_model(context)
        icon = _icon_for_tray_state(self.q_icon_type, context.state, self.current_variant)
    ...
    def handle_taskbar_created(self) -> None:
        ...
        self.set_state(self.current_context, variant=self.current_variant)
        self.show()
...
def create_agent_tray(
    ...
    *,
    ...
    icon = _icon_for_tray_state(qt.QIcon, TrayState.READY, select_tray_variant(app))
...
def _connect_left_click(tray: Any, on_action: TrayActionHandler, qt: TrayQtTypes) -> None:
    ...
    def handle_activated(reason: object) -> None:
        if trigger is None or reason == trigger:
            on_action(MenuAction.OPEN_SETPIECE)

    tray.activated.connect(handle_activated)


def _icon_for_tray_state(q_icon_type: Any, state: TrayState, variant: str) -> Any:
    asset = tray_icon_asset(state.value, variant=variant)
    return qt_icon_from_brand_asset(q_icon_type, asset.relative_path)
