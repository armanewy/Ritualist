...
class ResidentAgent:
    ...
    def handle_menu_action(self, action: MenuAction) -> None:
        if action == MenuAction.EXIT_SETPIECE:
    ...
    def resolve_pending_confirmation(self, decision: ConfirmationDecision) -> bool:
        ...
        if self.state.pending_confirmation is None:
            return False
        from setpiece.runtime_models import ConfirmationResolved, StepState
...
def start_agent(
    *,
    ...
    if not claim.is_primary:
        raise SetpieceError(claim.error or "Resident Agent could not start.")
...
def _ensure_application(qt: TrayQtTypes) -> Any:
    ...
    if instance is not None:
        apply_qt_application_icon(instance, qt.QIcon)
        return instance
    app = qt.QApplication([])
    apply_qt_application_icon(app, qt.QIcon)
    return app
...
def _intent_for_menu_action(action: MenuAction) -> ActivationIntent | None:
    mapping: MenuIntentMap = {
        MenuAction.OPEN_SETPIECE: ActivationIntent("open_picker"),
