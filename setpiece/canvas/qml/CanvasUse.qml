...
ApplicationWindow {
    ...
    height: 760
    visible: true
    title: "Setpiece Canvas"
    color: backgroundPassthrough ? "transparent" : token("background", "#070c13")

    property var canvasController: typeof setpieceCanvasUseController === "undefined" ? null : setpieceCanvasUseController
    property var canvasPayload: typeof setpieceCanvasPayload === "undefined" ? ({}) : setpieceCanvasPayload
    property var editPayload: canvasController ? canvasController.editPayload : setpieceCanvasEditPayload
    property var suggestionsPayload: canvasController ? canvasController.suggestionsPayload : ({})
    property var performanceSettings: typeof setpieceCanvasPerformance === "undefined" ? ({}) : setpieceCanvasPerformance
    property var hostSettings: typeof setpieceCanvasHost === "undefined" ? ({ mode: "windowed", taskbar_policy: "respect" }) : setpieceCanvasHost
    property bool e2eEnabled: typeof setpieceE2EEnabled === "undefined" ? false : setpieceE2EEnabled
    property bool editMode: canvasController ? canvasController.editMode : false
    property bool mockMode: typeof setpieceMockMode === "undefined" ? false : setpieceMockMode
    ...
    function activityLedgerSummary(item) {
        ...
        if (item.ownership_count !== undefined && item.ownership_count !== null && Number(item.ownership_count) > 0) {
            details.push(Number(item.ownership_count) + " Setpiece-opened resource" + (Number(item.ownership_count) === 1 ? "" : "s"))
        }
        return details.join(" | ")
    }
