...
from setpiece.e2e import enabled as e2e_enabled
...
from setpiece.home.confirmation import InlineConfirmationPresenter
from setpiece.home.models import HomeCardStatus, HomeRuntimeEvent
...
def run_canvas_use(
    ...
    *,
    ...
    try:
        ...
        raise DependencyMissingError("Canvas Use Mode requires PySide6; install setpiece[gui]") from exc
        ...
        def _ensure_executor(self) -> ThreadPoolExecutor:
            if self._executor is None:
                self._executor = ThreadPoolExecutor(
                    max_workers=2,
                    thread_name_prefix="setpiece-canvas-use",
                )
    ...
    engine.rootContext().setContextProperty("setpieceE2EEnabled", e2e_enabled())
    engine.rootContext().setContextProperty("setpieceCanvasHost", host_payload)
    engine.rootContext().setContextProperty("setpieceCanvasUseController", controller)
    engine.rootContext().setContextProperty("setpieceCanvasPayload", controller.payload)
    engine.rootContext().setContextProperty("setpieceCanvasEditPayload", controller.editPayload)
    engine.rootContext().setContextProperty("setpieceCanvasPerformance", performance)
    qml_resource = files("setpiece.canvas.qml").joinpath("CanvasUse.qml")
    with as_file(qml_resource) as qml_path:
        engine.load(QUrl.fromLocalFile(str(qml_path)))
        if not engine.rootObjects():
            raise SetpieceError(f"Canvas Use UI failed to load: {qml_path}")
...
def _apply_canvas_host(
    ...
    if screen is None:
        raise SetpieceError("Desktop Work-Area Canvas requires a Qt screen with available geometry.")
