...
from setpiece.shortcuts import ShortcutService, shortcut_kind_for_component, shortcut_request_from_component
...
class CanvasRuntimeController:
    ...
    def dispatch(
        ...
        *,
        ...
        raise SetpieceError(f"{component.id}: unsupported canvas action '{action}'")
    ...
    def _dispatch_recipe_action(
        ...
        *,
        ...
        if not recipe_ref:
            raise SetpieceError(f"{component.id}: recipe binding is required for {action}")
        if self.context.recipe_ids is not None and recipe_ref not in self.context.recipe_ids:
            raise SetpieceError(f"{component.id}: recipe binding '{recipe_ref}' is unresolved")
        if action == "run":
        ...
        if action in {"open_logs", "open_run_log"}:
            path = self.action_service.resolve_runs_path()
            return _success(component, action, f"run logs path resolved: {path}", {"path": str(path)})
        raise SetpieceError(f"{component.id}: unsupported recipe canvas action '{action}'")
    ...
    def _dispatch_controller_action(
        ...
        if action == "open_run_log":
            ...
            return _success(component, action, f"run logs path resolved: {path}", {"path": str(path)})
        if control is None:
            raise SetpieceError(f"{component.id}: no active runtime control is available")
        if action == "pause":
            control.pause()
        elif action == "resume":
            control.resume()
            ...
            raise SetpieceError(f"{component.id}: unsupported controller action '{action}'")
    ...
    def _dispatch_target_action(
        ...
        if action != "preview_plan":
            raise SetpieceError(f"{component.id}: target start execution is not implemented; use preview_plan")
        target = _target_reference(component)
        if not target:
            raise SetpieceError(f"{component.id}: target binding is required for preview_plan")
    ...
    def _dispatch_shortcut_action(
        ...
        if action != request.action_id:
            raise SetpieceError(f"{component.id}: unsupported shortcut action '{action}'")
...
def _find_component(document: CanvasDocument, component_id: str) -> CanvasComponent:
    for component in document.components:
        if component.id == component_id:
            return component
    raise SetpieceError(f"canvas component not found: {component_id}")
...
def _validate_action(component: CanvasComponent, action: str) -> None:
    ...
    try:
        ...
        raise SetpieceError(f"{component.id}: unknown component type '{component.type}'") from exc
    if action not in spec.actions:
        raise SetpieceError(f"{component.id}: unsupported canvas action '{action}'")
    if not action or any(marker in action.casefold() for marker in ("script", "shell", "powershell", "js")):
        raise SetpieceError(f"{component.id}: arbitrary canvas actions are not allowed")
