...
from setpiece.paths import canvases_dir, canvases_path
...
def load_bundled_canvas(canvas_id: str) -> CanvasDocument:
    ...
    if not text:
        raise SetpieceError("bundled canvas id must not be blank")
    for bundled in _bundled_canvas_paths():
        if bundled.stem == text:
            return _load_canvas_path(bundled)
    raise SetpieceError(f"bundled canvas not found: {text}")
    ...
    if not components:
        components.append(
            CanvasComponent(
                ...
                height=72,
                z=1,
                props={"text": "Run setpiece init to install recipes."},
            )
        )
...
def _mock_props(type_id: str) -> dict[str, object]:
    ...
    if type_id == "shortcut.folder":
        return {"title": "Mock folder shortcut", "path": "C:/Setpiece/MockFolder"}
    if type_id == "shortcut.app":
        return {"title": "Mock app shortcut", "path": "C:/Setpiece/MockApp.exe"}
...
def _mock_binding(type_id: str) -> CanvasComponentBinding | None:
    ...
    if type_id == "shortcut.folder":
        return CanvasComponentBinding(kind=CanvasBindingKind.SHORTCUT_FOLDER, path="C:/Setpiece/MockFolder")
    if type_id == "shortcut.app":
        return CanvasComponentBinding(kind=CanvasBindingKind.SHORTCUT_APP, path="C:/Setpiece/MockApp.exe")
    if type_id == "shortcut.url":
...
def _resolve_canvas_path(id_or_path: str | Path) -> Path:
    ...
    if not text:
        raise SetpieceError("canvas id or path must not be blank")
    ...
    for bundled in _bundled_canvas_paths():
        if bundled.stem == text:
            return bundled
    raise SetpieceError(f"canvas not found: {text}")
...
def _load_canvas_path(path: Path) -> CanvasDocument:
    try:
        ...
        raise SetpieceError(f"could not read canvas {path}: {exc}") from exc
    if not isinstance(raw, dict):
        raise SetpieceError(f"canvas must be a mapping: {path}")
    try:
        return CanvasDocument.model_validate(raw)
    except ValidationError as exc:
        raise SetpieceError(f"invalid canvas {path}: {exc}") from exc
...
def _bundled_canvas_paths() -> list[Path]:
    try:
        root = resources.files("setpiece.sample_canvases")
...
def canvas_show_payload(document: CanvasDocument, *, strict: bool = False) -> dict[str, Any]:
    ...
    return {
        "schema_version": "setpiece.canvas.show.v1",
...
def default_canvas_document() -> CanvasDocument:
    try:
        ...
        raise SetpieceError("bundled gaming_desktop canvas is missing") from exc
