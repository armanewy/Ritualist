...
class AssertFileExistsHandler:
    ...
    def run(self, step: AssertFileExistsStep, context: ActionContext) -> str:
        ...
        if not _wait_until(lambda: path.is_file(), timeout_seconds=step.timeout_seconds, context=context):
            if path.exists():
                raise SetpieceError(f"assert.file_exists failed: path is not a file: {path}")
            raise SetpieceError(f"assert.file_exists failed: file does not exist: {path}")
...
class AssertPathExistsHandler:
    ...
    def run(self, step: AssertPathExistsStep, context: ActionContext) -> str:
        ...
        if not _wait_until(path.exists, timeout_seconds=step.timeout_seconds, context=context):
            raise SetpieceError(f"assert.path_exists failed: path does not exist: {path}")
...
class AssertWindowExistsHandler:
    ...
    def run(self, step: AssertWindowExistsStep, context: ActionContext) -> str:
        ...
        if not _wait_until(
            ...
            raise SetpieceError(f"assert.window_exists failed: window not found: {target}")
...
def _read_registry_value(key: str, value_name: str) -> Any:
    ...
    try:
        ...
        raise SetpieceError(f"assert.registry_value failed: cannot read {key}: {exc}") from exc
...
def _parse_registry_key(key: str, winreg_module) -> tuple[Any, str]:
    ...
    if not separator or not subkey:
        raise SetpieceError("assert.registry_value key must include a hive and subkey")
    hives = {
    ...
    if hive is None:
        raise SetpieceError(f"assert.registry_value unsupported registry hive: {hive_name}")
