...
class BrowserWaitTextHandler:
    ...
    def run(self, step: BrowserWaitTextStep, context: ActionContext) -> str:
        ...
        if not _wait_until(
            ...
            raise SetpieceError(f"browser.wait_text timed out: visible text not found: {step.text}")
...
class BrowserWaitTitleHandler:
    ...
    def run(self, step: BrowserWaitTitleStep, context: ActionContext) -> str:
        ...
        if not _wait_until(
            ...
            raise SetpieceError(f"browser.wait_title timed out: title not found: {target}")
...
class BrowserWaitUrlHandler:
    ...
    def run(self, step: BrowserWaitUrlStep, context: ActionContext) -> str:
        ...
        if not _wait_until(
            ...
            raise SetpieceError(f"browser.wait_url timed out: URL not found: {target}")
