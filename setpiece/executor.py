...
class WorkflowExecutor:
    def __init__(
        ...
        *,
        ...
        self.logger = logger or logging.getLogger("setpiece")
...
def _final_run_message(state: RunState, *, stopped_reason: str | None) -> str:
    ...
    if stopped_reason == INTERRUPTED_REASON:
        return "Setpiece exited before finalizing this run."
    if state == RunState.SUCCESS:
        return "Run completed."
    if state == RunState.STOPPED:
        return "Run stopped."
...
def _owned_resource_for_step(step: ExecutableStep, *, keep_open_active: bool) -> dict[str, Any] | None:
    ...
    if step.action == "browser.media":
        return {
            "kind": "media",
            "description": "Setpiece-started browser media",
            "owned_by_ritual": True,
    ...
    if step.action == "app.launch":
        return {
            "kind": "app",
            "description": f"App launched by Setpiece: {_safe_command_label(getattr(step, 'command', ''))}",
            "owned_by_ritual": True,
    ...
    if step.action in WINDOW_LAYOUT_ACTIONS:
        return {
            "kind": "window_layout",
            "description": f"Window layout changed by Setpiece: {step.action}",
            "owned_by_ritual": True,
            "cleanup_available": False,
...
def _cleanup_offer(
    *,
    ...
    return {
        ...
        "options": options,
        "message": "Choose cleanup after the stop; Setpiece will not claim to undo irreversible changes.",
    }
...
def _preview_label(step: ExecutableStep) -> str | None:
    if isinstance(step, DesktopClickTextStep):
        return f"Setpiece: clicking {step.text}"
    if step.action == "window.focus":
        return "Setpiece: focusing window"
    if step.action == "window.minimize":
        return "Setpiece: minimizing window"
    if step.action == "window.move":
        return "Setpiece: moving window"
    if step.action == "window.resize":
        return "Setpiece: resizing window"
    if step.action == "window.maximize":
        return "Setpiece: maximizing window"
    if step.action == "window.restore":
        return "Setpiece: restoring window"
    if step.action == "window.snap_left":
        return "Setpiece: snapping window left"
    if step.action == "window.snap_right":
        return "Setpiece: snapping window right"
    if step.action == "window.snap_top":
        return "Setpiece: snapping window top"
    if step.action == "window.snap_bottom":
        return "Setpiece: snapping window bottom"
    return None
...
def _confirmation_safety_message(step: ExecutableStep) -> str | None:
    ...
    if getattr(step, "requires_confirmation", False):
        return "This step requires explicit confirmation before Setpiece continues."
    return None
