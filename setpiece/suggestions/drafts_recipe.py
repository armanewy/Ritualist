...
from setpiece.actions.metadata import ActionMetadata
from setpiece.actions.registry import ActionRegistry, create_default_registry
from setpiece.models import AssertionStep, Recipe, WorkflowStep
...
from setpiece.templating import collect_template_variables, render_template_data


DRAFT_RECIPE_SCHEMA_VERSION = "setpiece.suggestion.recipe_draft.v1"
...
def _diagnostic_value(action: str, field_name: str) -> Any:
    ...
    if field_name in {"condition", "when"}:
        return {"type": "path.exists", "path": "SETPIECE_DRAFT_PLACEHOLDER"}
    if field_name == "args":
        return []
    if field_name == "env":
        return {}
