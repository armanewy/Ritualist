...
from .errors import RecipeValidationError, SetpieceError
...
HOME_PLAN_SUMMARY_SCHEMA_VERSION = "intent.home_summary.v1"
...
class HomePlanSummary:
    ...
    schema_version: str = HOME_PLAN_SUMMARY_SCHEMA_VERSION
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "estimated_step_count": self.estimated_step_count,
            "expected_confirmations": list(self.expected_confirmations),
        }


class IntentPlanCompiler:
    """Deterministic, rule-based compiler from IntentSpec to PrimitivePlan."""

    def __init__(self, *, primitive_registry: PrimitiveRegistry | None = None) -> None:
        self.primitive_registry = primitive_registry or create_primitive_registry()

    def compile(self, intent: IntentSpec) -> PrimitivePlan:
        return _compile_intent_to_plan(intent, primitive_registry=self.primitive_registry)
...
def compile_intent_to_plan(
    ...
    *,
    ...
    return IntentPlanCompiler(primitive_registry=primitive_registry).compile(intent)


def _compile_intent_to_plan(
    intent: IntentSpec,
    *,
    primitive_registry: PrimitiveRegistry | None = None,
) -> PrimitivePlan:
    ...
    if intent.kind == "target.start":
        if intent.constraints.get("fixture") == "placeholder":
            unresolved.append("target.start placeholder requires choosing a concrete target id or name")
            unresolved.append("Target Resolution will compile launch options after a target is supplied")
...
def plan_preview_payload(plan: PrimitivePlan, doctor: PlanDoctorReport) -> dict[str, object]:
    return {
        ...
        "home_summary": build_home_plan_summary(plan, doctor).to_dict(),
    }


def build_home_plan_summary(plan: PrimitivePlan, doctor: PlanDoctorReport) -> HomePlanSummary:
    intent = plan.intent or {}
    title = str(intent.get("display_name") or plan.recipe_id or plan.plan_id)
    summary = str(
        intent.get("user_visible_summary")
        or intent.get("requested_outcome")
        or f"Preview {plan.plan_id}."
    )
    policy_status = "allowed" if doctor.policy_report.allowed else "blocked"
    return HomePlanSummary(
        card_title=title,
        user_visible_summary=summary,
        risk_summary=dict(plan.risk_summary),
        doctor_status=doctor.compatibility,
        policy_status=policy_status,
        unresolved_questions=tuple(plan.unresolved_questions),
        estimated_step_count=len(plan.steps),
        expected_confirmations=tuple(plan.confirmations_needed),
    )
...
def _intent_from_mapping(raw: dict[str, Any]) -> IntentSpec:
    ...
    kind_value = normalized.get("kind")
    if isinstance(kind_value, str):
        fixture = _intent_fixture_from_reference(kind_value)
        if fixture is not None:
            fixture_data = fixture.model_dump(mode="python")
            for key, value in normalized.items():
                if key == "kind":
                    continue
                if key in {"constraints", "preferences"} and isinstance(value, dict):
                    merged = dict(fixture_data.get(key) or {})
                    merged.update(value)
                    fixture_data[key] = merged
                    continue
                fixture_data[key] = value
            normalized = fixture_data
    ...
    try:
        ...
        raise SetpieceError(str(exc)) from exc


def _intent_from_kind(kind: str) -> IntentSpec:
    cleaned = kind.strip()
    if not cleaned:
        raise SetpieceError("intent kind must not be blank")
    fixture = _intent_fixture_from_reference(cleaned)
    if fixture is not None:
        return fixture
...
def _intent_fixture_from_reference(reference: str) -> IntentSpec | None:
    kind, separator, fixture = reference.partition(":")
    if not separator:
        return None
    fixture = fixture.strip().replace("-", "_")
    kind = kind.strip()
    if kind == "diagnostics.collect" and fixture in {"minimal", "support", "gamer_crash"}:
        display_preset = fixture.replace("_", " ")
        return IntentSpec(
            intent_id=f"diagnostics_collect_{fixture}",
            kind=kind,
            display_name=f"Collect {display_preset} diagnostics",
            description="Built-in deterministic diagnostics collection preview.",
            requested_outcome=f"Collect {display_preset} diagnostics artifacts.",
            constraints={"preset": fixture},
            risk_budget=PrimitiveRisk.READ_ONLY.value,
            user_visible_summary=f"Collect {display_preset} diagnostics without secrets.",
        )
    if kind == "workspace.prepare" and fixture == "basic":
        return IntentSpec(
            intent_id="workspace_prepare_basic",
            kind=kind,
            display_name="Prepare workspace",
            description="Built-in deterministic workspace preparation preview.",
            requested_outcome="Prepare a local workspace after apps/windows are configured.",
            constraints={"fixture": "basic"},
    ...
    if kind == "target.start" and fixture == "placeholder":
        return IntentSpec(
            intent_id="target_start_placeholder",
            kind=kind,
            display_name="Start target",
            description="Built-in placeholder target start preview.",
            requested_outcome="Resolve a local target after the target id is configured.",
            constraints={"fixture": "placeholder"},
            risk_budget=PrimitiveRisk.CONTROLS_UI.value,
