...
def _check_capability(capability: str) -> DoctorCheck:
    if capability == "playwright":
        return _check_import(
            "playwright.sync_api",
            "playwright",
            "install setpiece[browser] and run 'python -m playwright install chromium'",
        )
    ...
    if capability == "windows_uia":
        if sys.platform != "win32":
            return DoctorCheck(
                "error",
                capability,
                "Windows UI Automation requires Windows and pywinauto; install setpiece[windows]",
                section="Capabilities",
                details={"capability": capability},
            )
        return _check_import("pywinauto", capability, "install setpiece[windows]")
    if capability == "window_management":
        if sys.platform != "win32":
            return DoctorCheck(
                "error",
                capability,
                "window management requires Windows UI Automation; install setpiece[windows]",
                section="Capabilities",
                details={"capability": capability},
            )
    ...
    if capability == "keyboard_input":
        if sys.platform != "win32":
            return DoctorCheck(
                "error",
                capability,
                "keyboard input requires Windows UI Automation; install setpiece[windows]",
                section="Capabilities",
                details={"capability": capability},
            )
        return _check_import("pywinauto", capability, "install setpiece[windows]")
    ...
    if capability == "process_inspection":
        check = _check_import("psutil", "psutil", "install setpiece[windows]")
        return DoctorCheck(
            check.status,
            capability,
            check.message,
...
def _check_window_management_capability() -> DoctorCheck:
    pywinauto_check = _check_import("pywinauto", "window_management", "install setpiece[windows]")
    ...
    if not _module_available("win32api"):
        return DoctorCheck(
            "error",
            "window_management",
            "window_management import failed; install setpiece[windows] (pywin32/win32api missing)",
            section="Capabilities",
        ...
        if not _module_available("pywinauto"):
            checks.append(
                DoctorCheck(
                    "warn",
                    "expected_window",
                    f"cannot inspect expected window '{target}'; install setpiece[windows]",
                    section="Windows/UI labels",
                )
            )
            continue
        ...
        if not _module_available("pywinauto"):
            checks.append(
                DoctorCheck(
                    ...
                    "expected_label",
                    "cannot inspect expected label "
                    f"'{expected.text}' in '{expected.window_title_contains}'; install setpiece[windows]",
                    section="Windows/UI labels",
                )
            )
            continue
