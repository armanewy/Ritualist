...
from .errors import DependencyMissingError, PlatformUnsupportedError, SetpieceError
...
def run_read_only_primitive(
    ...
    *,
    ...
    raise SetpieceError(f"no read-only primitive executor for {primitive_id}")
...
def _process_primitive(verb: str, parameters: dict[str, Any]) -> PrimitiveExecutionResult:
    ...
    if verb in {"wait_running", "wait_exit"}:
        ...
        while True:
            ...
            time.sleep(0.25)
    raise SetpieceError(f"unsupported app.process primitive verb: {verb}")
...
def _window_primitive(
    ...
    *,
    ...
    raise SetpieceError(f"unsupported window.topology primitive verb: {verb}")
...
def _uia_primitive(
    ...
    *,
    ...
    raise SetpieceError(f"unsupported uia.element primitive verb: {verb}")
...
def _browser_assert_primitive(
    ...
    *,
    ...
    if verb == "text_visible":
        ...
        raise SetpieceError(f"unsupported browser.assert primitive verb: {verb}")
...
def _runtime_assert_primitive(verb: str, parameters: dict[str, Any]) -> PrimitiveExecutionResult:
    ...
    raise SetpieceError(f"unsupported runtime.assert primitive verb: {verb}")
...
def _hardware_primitive(verb: str, parameters: dict[str, Any]) -> PrimitiveExecutionResult:
    ...
    raise SetpieceError(f"unsupported hardware.inventory primitive verb: {verb}")
...
def _network_primitive(verb: str, parameters: dict[str, Any]) -> PrimitiveExecutionResult:
    ...
    raise SetpieceError(f"unsupported network.connectivity primitive verb: {verb}")
...
def _diagnostics_primitive(verb: str, parameters: dict[str, Any]) -> PrimitiveExecutionResult:
    ...
    if preset is None:
        raise SetpieceError(f"unsupported diagnostics.bundle primitive verb: {verb}")
...
def _psutil() -> Any:
    try:
        ...
        raise DependencyMissingError(
            "process inventory requires optional dependency psutil; install setpiece[windows]"
        ) from exc
    return psutil
...
def _required_text(parameters: dict[str, Any], name: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise SetpieceError(f"primitive parameter '{name}' is required")
    return value.strip()
...
def _bool_parameter(parameters: dict[str, Any], name: str, default: bool) -> bool:
    ...
    if text in {"0", "false", "no", "off"}:
        return False
    raise SetpieceError(f"primitive parameter '{name}' must be a boolean")
...
def _float_parameter(parameters: dict[str, Any], name: str, default: float) -> float:
    ...
    try:
        ...
        raise SetpieceError(f"primitive parameter '{name}' must be a number") from exc
...
def _optional_int_parameter(parameters: dict[str, Any], name: str) -> int | None:
    ...
    try:
        ...
        raise SetpieceError(f"primitive parameter '{name}' must be an integer") from exc
