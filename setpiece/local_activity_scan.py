...
from setpiece.activity_journal import ActivityJournal, JournalEvent
...
class ActivityJournalCollector:
    ...
    path: Path | None = None
    limit: int = 10
    collector_id: str = SETPIECE_JOURNAL_SOURCE_ID
            ...
            return ActivityCollectionResult(
                ...
                warnings=(
                    ActivityWarning(
                        ...
                        source_id=SETPIECE_JOURNAL_SOURCE_ID,
                        message=f"Setpiece activity journal is unavailable: {exc}",
                    ),
                ),
            )
...
def build_local_activity_collectors(
    ...
    if SETPIECE_JOURNAL_SOURCE_ID in source_ids:
...
def _journal_collector(request: LocalActivityScanRequest) -> ActivityCollector:
    ...
    return SetpieceJournalCollector(
        limit=request.max_journal_events,
