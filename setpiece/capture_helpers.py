...
class CaptureHelperController:
    ...
    def _path_picker(self) -> PathPicker:
        if self.path_picker is None:
            raise SetpieceError("path capture requires an explicit path picker")
...
def _choose_label(
    ...
    *,
    ...
    if (text is None) == (label_index is None):
        raise SetpieceError("choose visible text with exactly one of text or label_index")
    labels = inspection.labels
    if label_index is not None:
        if label_index < 0 or label_index >= len(labels):
            raise SetpieceError(f"visible text index out of range: {label_index}")
        return labels[label_index]
    assert text is not None
    for label in labels:
        if label == text:
            return label
    raise SetpieceError(f"visible text was not present in the inspection: {text}")
...
def _require_value(value: str | None, label: str) -> str:
    ...
    if not text:
        raise SetpieceError(f"{label} is empty")
    return text
