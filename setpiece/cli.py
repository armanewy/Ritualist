...
perf_app = typer.Typer(help="Measure Setpiece CLI operations without timing gates.")
...
settings_app = typer.Typer(help="Inspect and manage local Setpiece settings.")
...
canvas_app = typer.Typer(help="Inspect and validate local Setpiece Canvas documents.")
...
theme_app = typer.Typer(help="Inspect and validate safe declarative Setpiece themes.")
room_app = typer.Typer(help="Inspect starter Setpiece Rooms backed by Canvas templates.")
...
@app.command()
def init() -> None:
    """Create Setpiece app directories and install bundled sample recipes."""
    report = initialize_app()
    console.print("[green]Setpiece initialized.[/]")
...
def list_recipes() -> None:
    ...
    console.print(table)
    if not rows:
        console.print("No recipes found. Run [bold]setpiece init[/] first.")


@app.command()
def paths() -> None:
    """Show Setpiece's local data directories."""
    _print_paths(
        {
...
@app.command("migrate-legacy-data")
def migrate_legacy_data_command(
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Inspect without copying files.")] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Print machine-readable JSON.")] = False,
) -> None:
    """Inspect or run nondestructive legacy app data migration."""

    report = migrate_legacy_data(dry_run=dry_run)
    if json_output:
        console.print_json(data=report)
        return
    console.print(f"Status: {report['status']}")
    console.print(f"Legacy data: {report['legacy_data_root']}")
    console.print(f"Setpiece data: {report['setpiece_data_root']}")
    console.print(f"Operations: {len(report['operations'])}")
...
def learning_enable(
    source_ids: Annotated[
        ...
        typer.Option(
            "--source",
            "-s",
            help="Explicitly select an allowed source: setpiece_journal, open_windows, or recent_items.",
        ),
    ] = None,
    json_output: Annotated[
        bool,
...
def learning_journal(
    ...
    """Show local Setpiece-owned learning journal events."""
...
def _print_recipe_view(payload: dict[str, Any]) -> None:
    ...
    if confirmations:
        ...
        console.print("  None")
    console.print("Setpiece will never:")
    for item in payload.get("what_setpiece_will_never_do", []):
        console.print(f"  - {escape(str(item))}")
...
def target_plan(
    ...
    except (SetpieceError, ValueError) as exc:
        console.print(f"[red]Error:[/] {escape(str(exc))}")
        raise typer.Exit(1) from exc
    if json_output:
...
def recipe_edit_setup(
    ...
    overrides_dir: Annotated[
        ...
        typer.Option(
            "--overrides-dir",
            help="Directory for setup override YAML. Defaults to Setpiece app config.",
        ),
    ] = None,
    json_output: Annotated[
        bool,
...
def room_list(
    ...
    if json_output:
        console.print_json(data=payload)
        return
    table = Table(title="Setpiece Rooms")
    table.add_column("ID")
    table.add_column("Name")
...
def canvas_list(
    ...
    if json_output:
        ...
        return
    table = Table(title="Setpiece Canvases")
    table.add_column("ID")
    table.add_column("Name")
    ...
    console.print(table)
    if not rows:
        console.print("No canvases found. Run [bold]setpiece canvas init[/] first.")
...
def canvas_pack_export(
    ...
    out: Annotated[
        Path,
        typer.Option("--out", help="Output .setpiececanvas archive path."),
    ],
    readme: Annotated[
        Path | None,
...
def canvas_pack_import(
    pack: Annotated[Path, typer.Argument(help="Path to a .setpiececanvas archive.")],
    json_output: Annotated[
        bool,
...
def canvas_theme_export(
    ...
    out: Annotated[
        Path,
        typer.Option("--out", help="Output .setpiecetheme archive path."),
    ],
    readme: Annotated[
        Path | None,
        typer.Option("--readme", help="Optional UTF-8 README file to include as README.md."),
...
def canvas_theme_import(
    pack: Annotated[Path, typer.Argument(help="Path to a .setpiecetheme archive.")],
    json_output: Annotated[
        bool,
        typer.Option("--json", help="Print machine-readable import record."),
    ] = False,
...
def suite_pack_export(
    canvas_pack: Annotated[
        Path,
        typer.Option("--canvas-pack", help="Nested .setpiececanvas Room pack to include."),
    ],
    out: Annotated[
        Path,
        typer.Option("--out", help="Output .setpiecesuite archive path."),
    ],
    suite_id: Annotated[
        str | None,
    ...
    theme_pack: Annotated[
        Path | None,
        typer.Option("--theme-pack", help="Optional nested .setpiecetheme pack to include."),
    ] = None,
    ritual_pack: Annotated[
        list[Path] | None,
        typer.Option("--ritual-pack", help="Optional behavior-bearing .setpiecepack to include."),
    ] = None,
    readme: Annotated[
        Path | None,
        typer.Option("--readme", help="Optional UTF-8 README file to include as README.md."),
...
def suite_pack_validate(
    suite: Annotated[Path, typer.Argument(help="Path to a .setpiecesuite archive.")],
    json_output: Annotated[
        bool,
        typer.Option("--json", help="Print machine-readable validation result."),
    ] = False,
...
def theme_list(
    ...
    if json_output:
        console.print_json(data=[row.to_dict() for row in rows])
        return
    table = Table(title="Setpiece Themes")
    table.add_column("ID")
    table.add_column("Name")
    table.add_column("Source")
    table.add_column("Path")
...
def policy_check(
    target: Annotated[str, typer.Argument(help="Recipe id/path, imported pack dir, or .setpiecepack.")],
    profile: Annotated[
        str,
...
def pack_export(
    ...
    out: Annotated[
        Path,
        typer.Option("--out", help="Output .setpiecepack archive path."),
    ],
    readme: Annotated[
        Path | None,
        typer.Option("--readme", help="Optional UTF-8 README file to include as README.md."),
    ...
    """Export a recipe into a portable .setpiecepack zip."""
    try:
...
def pack_import(
    pack: Annotated[str, typer.Argument(help="Path to a .setpiecepack archive.")],
) -> None:
    ...
    for recipe in record.recipes:
        ...
        console.print(f"- setpiece doctor {escape(str(path))}")
        console.print(f"- setpiece dry-run {escape(str(path))}")
    console.print(f"Enable after review with: setpiece pack enable {escape(record.import_id)}")
...
def pack_enable(
    import_id: Annotated[str, typer.Argument(help="Import id from 'setpiece pack list-imports'.")],
) -> None:
...
def perf_home_model(
    ...
    with measure_operation("perf.home-model") as report:
        from setpiece.home.models import create_mock_home_model
...
def show_run(
    run_id_or_path: Annotated[str, typer.Argument(help="Run id from 'setpiece runs' or run path.")],
    repair: Annotated[
        bool,
...
def _print_paths(paths: dict[str, object]) -> None:
    table = Table(title="Setpiece Paths")
    table.add_column("Name")
    table.add_column("Path")
...
def _build_policy_report_for_target(target: str, *, profile: str) -> PolicyReport:
    ...
    if candidate.exists() and candidate.is_file() and candidate.suffix == ".setpiecepack":
...
def _keep_alive_until_interrupted() -> None:
    console.print(
        "[yellow]Browser keep-open requested. Press Ctrl+C to let Setpiece exit.[/]"
    )
    try:
        while True:
            time.sleep(3600)
