...
from setpiece.paths import themes_path

THEME_SCHEMA_VERSION = "setpiece.theme.v1"
THEME_VALIDATION_SCHEMA_VERSION = "setpiece.theme.validation.v1"
...
APP_DEFAULT_TOKENS: dict[str, str | int | float] = {
    ...
    "color.success_panel": "#12251f",
    "color.warning_panel": "#252014",
    "color.danger_panel": "#28151c",
    "color.focus_panel": "#132235",
    "color.text": "#f5f7fb",
...
THEME_ACCESSIBILITY_SCHEMA_VERSION = "setpiece.theme.accessibility.v1"
...
def theme_show_payload(theme: ThemeDocument, *, theme_dir: Path | None = None) -> dict[str, Any]:
    ...
    return {
        "schema_version": "setpiece.theme.show.v1",
        "theme": theme.to_dict(),
...
def _resolve_theme_path(id_or_path: str | Path) -> Path:
    ...
    if not text:
        raise SetpieceError("theme id or path must not be blank")
    ...
    raise SetpieceError(f"theme not found: {text}")
...
def _load_theme_path(path: Path) -> ThemeDocument:
    try:
        ...
        raise SetpieceError(f"could not read theme {path}: {exc}") from exc
    if not isinstance(raw, dict):
        raise SetpieceError(f"theme must be a mapping: {path}")
    try:
        return ThemeDocument.model_validate(raw)
    except ValidationError as exc:
        raise SetpieceError(f"invalid theme {path}: {exc}") from exc
