...
PACK_SCHEMA_V1 = "setpiece.pack.v1"
LEGACY_PACK_SCHEMA_V1 = "ritualist.pack.v1"
SUPPORTED_PACK_SCHEMAS = frozenset({PACK_SCHEMA_V1, LEGACY_PACK_SCHEMA_V1, "v1"})

MANIFEST_NAME = "manifest.yaml"
RECIPE_NAME = "recipe.yaml"
README_NAME = "README.md"
ASSETS_PREFIX = "assets/"
PACK_EXTENSION = ".setpiecepack"
LEGACY_PACK_EXTENSION = ".ritualistpack"
SUPPORTED_PACK_EXTENSIONS = frozenset({PACK_EXTENSION, LEGACY_PACK_EXTENSION})
IMPORT_RECORD_SCHEMA = "setpiece.import.v1"
...
class PackValidationError(RecipeValidationError):
    """Raised when a Setpiece recipe pack archive or manifest is invalid."""


class PackImportError(SetpieceError):
...
class PackManifest(BaseModel):
    ...
    @field_validator("name", "version", "required_setpiece_version")
    @classmethod
...
def validate_pack(
    ...
    *,
    ...
    """Validate a Setpiece recipe pack archive without extracting or executing its contents."""

    pack_path = Path(path)
    _require_supported_pack_extension(pack_path)
    ...
    try:
        ...
        raise PackValidationError(f"invalid recipe pack zip: {exc}") from exc
...
def export_recipe_pack(
    ...
    *,
    ...
    """Export a validated recipe into a portable Setpiece pack archive."""
...
def build_pack_manifest(
    *,
    ...
    return PackManifest(
        ...
        required_setpiece_version=f">={__version__}",
...
def import_pack(pack: str | Path, *, registry: ActionRegistry | None = None) -> ImportedPackRecord:
    """Validate and copy a recipe pack into quarantine without running it."""
...
def enable_import(import_id: str, *, registry: ActionRegistry | None = None) -> ImportedPackRecord:
    ...
    if blocking_doctor_errors:
        raise PackImportError(
            f"doctor validation failed for {pack.recipe.id}; run 'setpiece doctor {record.recipe_path}'"
        )
...
def _validation_default_for_variable(name: str, value: Any) -> Any:
    ...
    if _looks_like_url_variable(name, value):
        return f"https://example.invalid/setpiece-required/{safe_name}"
...
def _require_supported_pack_extension(path: Path) -> None:
    if path.suffix.lower() not in SUPPORTED_PACK_EXTENSIONS:
        raise PackValidationError(
            f"pack path must end with {PACK_EXTENSION}; legacy {LEGACY_PACK_EXTENSION} is import-only"
        )
