...
from .canvas_packs import (
    ...
    CANVAS_PACK_EXTENSIONS,
    LEGACY_SUITE_PACK_EXTENSION,
    SUITE_PACK_EXTENSION,
    THEME_PACK_EXTENSION,
    THEME_PACK_EXTENSIONS,
    VisualPackError,
    import_canvas_pack,
    import_theme_pack,
)
...
from .packs import PACK_EXTENSION, SUPPORTED_PACK_EXTENSIONS, PackValidationError, import_pack, validate_pack
from .paths import imported_suite_packs_dir, imported_suite_packs_path

SUITE_PACK_SCHEMA = "setpiece.suite_pack.v1"
LEGACY_SUITE_PACK_SCHEMA = "ritualist.suite_pack.v1"
SUITE_PACK_SCHEMAS = frozenset({SUITE_PACK_SCHEMA, LEGACY_SUITE_PACK_SCHEMA})
SUITE_IMPORT_SCHEMA = "setpiece.suite_import.v1"
MANIFEST_NAME = "manifest.yaml"
README_NAME = "README.md"
SUITE_COPY_NAME = "suite.setpiecesuite"
SUITE_PACK_EXTENSIONS = frozenset({SUITE_PACK_EXTENSION, LEGACY_SUITE_PACK_EXTENSION})
...
class SuitePackError(SetpieceError):
    """Raised when a Setpiece suite archive or manifest is invalid."""
...
class SuitePackContents(BaseModel):
    ...
    def validate_canvas_entry(cls, value: SuitePackEntry) -> SuitePackEntry:
        if not value.path.startswith(CANVAS_PACK_PREFIX) or not _ends_with_any(value.path, CANVAS_PACK_EXTENSIONS):
            raise ValueError("suite canvas entry must be a supported canvas pack under packs/canvas/")
    ...
    def validate_theme_entry(cls, value: SuitePackEntry | None) -> SuitePackEntry | None:
        if value is None:
            return None
        if not value.path.startswith(THEME_PACK_PREFIX) or not _ends_with_any(value.path, THEME_PACK_EXTENSIONS):
            raise ValueError("suite theme entry must be a supported theme pack under packs/theme/")
        if value.behavior_bearing:
    ...
    def validate_ritual_entries(cls, value: list[SuitePackEntry]) -> list[SuitePackEntry]:
        seen: set[str] = set()
        for entry in value:
            if not entry.path.startswith(RITUAL_PACK_PREFIX) or not _ends_with_any(entry.path, SUPPORTED_PACK_EXTENSIONS):
                raise ValueError("suite ritual entries must be supported recipe pack files under packs/rituals/")
...
class SuitePackManifest(BaseModel):
    ...
    def validate_schema(cls, value: str) -> str:
        if value not in SUITE_PACK_SCHEMAS:
...
def import_suite_pack(
    ...
    *,
    ...
    try:
        ...
        with TemporaryDirectory(prefix="setpiece-suite-import-") as temp:
            temp_root = Path(temp)
...
def _validate_nested_packs(
    ...
    with TemporaryDirectory(prefix="setpiece-suite-validate-") as temp:
        temp_root = Path(temp)
        nested = [
...
def _validated_canvas_summary(path: Path, entry: str) -> NestedPackSummary:
    with TemporaryDirectory(prefix="setpiece-suite-export-") as temp:
...
def _read_suite_zip(path: Path) -> dict[str, bytes]:
    if path.suffix.casefold() not in SUITE_PACK_EXTENSIONS:
...
def _entry_allowed_by_layout(name: str) -> bool:
    ...
    if name.startswith(CANVAS_PACK_PREFIX):
        return _ends_with_any(name, CANVAS_PACK_EXTENSIONS)
    if name.startswith(THEME_PACK_PREFIX):
        return _ends_with_any(name, THEME_PACK_EXTENSIONS)
    if name.startswith(RITUAL_PACK_PREFIX):
        return _ends_with_any(name, SUPPORTED_PACK_EXTENSIONS)
    return False
...
def _ends_with_any(value: str, suffixes: frozenset[str]) -> bool:
    text = value.casefold()
    return any(text.endswith(suffix.casefold()) for suffix in suffixes)
