# Ritualist Binding Product Design

This document is the canonical repository contract for the accepted Ritualist
...
- `Ritualist.exe` opens Home by default.
...
Ritualist runs as one resident per-user local Agent. At rest there is no visible
application window. The user invokes Ritualist through the tray icon, the
...
Left-clicking the tray icon invokes Ritualist:
...
Exit is a separate explicit command named `Exit Ritualist`. If no attended run
...
Ritualist uses a light, warm, mostly opaque default expression called
Ritualist Paper. The visual system is quiet and task-focused, not decorative.
